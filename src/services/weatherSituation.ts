...
export function createDemoWeatherFixture(
  ...
  return {
    ...
    observed: {
      ...
      windSpeedKph: 12.0,
      freshness: 'FRESH',
      provenance: 'Observed meteorological ground telemetry',
    },
    forecast: {
      source: 'Open-Meteo (Numerical Prediction)',
      validAt: new Date(Date.now() + 3600 * 1000).toISOString(),
      retrievedAt: new Date().toISOString(),
      precipitationMm: 0.0,
      ...
      windSpeedKph: 14.0,
      freshness: 'FRESH',
      provenance: 'Numerical weather prediction model forecast',
    },
    officialWarning: {
      present: false,
      source: null,
