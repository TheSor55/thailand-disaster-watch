...
| Open-Meteo weather forecasts | Open-Meteo | [Official Terms](https://open-meteo.com/en/features#terms): CC BY 4.0; requires attribution | Controlled local technical pilot; no production display | `PENDING — TECHNICAL CONNECTIVITY VERIFIED` |
| RainViewer radar imagery | RainViewer | [API Terms](https://www.rainviewer.com/api/weather-maps-api.html): Free tier allows web display with mandatory attribution | Controlled local development preview; no production display | `PENDING — CONTROLLED PREVIEW ONLY` |
